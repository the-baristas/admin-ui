...
export class FlightComponent implements OnInit {
    ...
    public onUpdateFlight() {
        this.flightService.updateFlight(this.editFlight as Flight).subscribe(
            (response: any) => {
                const pageIndex = this.pageNumber - 1;
    ...
    public onAddFlight() {
        this.flightService
            .addFlight(this.addFlightForm.value as Flight)
            .subscribe(
                (response: any) => {
                    const pageIndex = this.pageNumber - 1;
    ...
    public onDeleteFlight() {
        this.confirmation = confirm(
            'Are you sure you want to delete this flight? (For auditing purposes, flights should be disabled rather than deleted.'
        );

        if (this.confirmation === true) {
            console.log(this.editFlight.id);
            this.flightService.deleteFlight(this.editFlight.id).subscribe(
                (response: any) => {
                    const pageIndex = this.pageNumber - 1;
                    this.flightService
                        .getFlightsPage(pageIndex, this.pageSize)
    ...
    setPage(pageNo: number) {
        if (pageNo < 1 || pageNo > this.totalFlights) {
            ...
            console.log(pageNo);
            this.flightService
                .getFlightsPage(pageNo - 1, this.pageSize)
                .subscribe((flightsPage: Page<Flight>) => {
                    this.currentPage = flightsPage;
                    this.pageNumber = flightsPage.number + 1;
                    this.flights = flightsPage.content;
    ...
    searchFlightsById() {
        ...
        this.flightService
            .getFlight(parseInt(this.searchFlightsForm.value.searchString))
            .subscribe(
                (response: Flight) => {
    ...
    searchFlightsByLocation() {
        if (
            this.searchFlightsForm.value.searchOrigin === '' ||
            this.searchFlightsForm.value.searchDestination === ''
        ) {
            let div: any = document.getElementById('searchByIdErrorMessage');
            div.style.display = 'none';
            const pageIndex = this.pageNumber - 1;
