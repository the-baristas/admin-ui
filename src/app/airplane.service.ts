...
export class AirplaneService {
    private airplaneServicePath: string = '/airplanes';
    ...
    getAirplanes(): Observable<Airplane[]> {
        return this.http.get<Airplane[]>(environment.apiUrl + this.airplaneServicePath)
            .pipe(
    ...
    getAirplaneNo404<Data>(id: number): Observable<Airplane> {
        const url = `${environment.apiUrl + this.airplaneServicePath}/?id=${id}`;
    ...
    searchAirplanes(term: string): Observable<Airplane[]> {
        ...
        return this.http.get<Airplane[]>(`${environment.apiUrl + this.airplaneServicePath}/?model=${term}`).pipe(
            tap(x =>
                x.length
    ...
    addAirplane(airplane: Airplane): Observable<Airplane> {
        return this.http.post<Airplane>(environment.apiUrl + this.airplaneServicePath, airplane, this.httpOptions).pipe(
    ...
    updateAirplane(airplane: Airplane): Observable<any> {
        return this.http.put(environment.apiUrl + this.airplaneServicePath, airplane, this.httpOptions).pipe(
